...
void RobotCtrl::resetToInitialPosition() {
    if (robot_ && !init_offset_.empty()) {
        ...
        if (!init_offset_.empty()) {
            ...
            for (size_t i = 0; i < init_offset_.size() && i < initial_theta.size(); ++i) {
...
std::vector<std::tuple<double, double, double>> RobotCtrl::smoothPath(
    ...
    if (with_obstacle_avoidance && map3d_ && path_smoother_) {
        smoothed_path_ = path_smoother_->smooth_path_with_obstacle_avoidance(
            path_to_smooth, map3d_, smoothing_factor, num_points);
    } else if (path_smoother_) {
        smoothed_path_ = path_smoother_->smooth_path(path_to_smooth, smoothing_factor, num_points);
    } else {
        return {};
    }
...
std::vector<std::vector<double>> smoothJointAngles(
    ...
    // 校验输入参数维度一致性
    size_t num_joints = thetas[0].size();
    ...
    // 辅助函数：检查单个关节角度是否在限位内
    auto is_joint_within_limits = [&](double angle, size_t joint_idx) {
        return (angle >= theta_min[joint_idx]) && (angle <= theta_max[joint_idx]);
    };
    ...
    for (size_t i = 1; i < thetas.size(); ++i) {
        ...
        for (size_t j = 0; j < num_joints; ++j) {
            if(is_joint_within_limits(curr[j], j)){
                adjusted_curr[j] = curr[i];
            }
            else{
                if(curr[j] > theta_max[j]){
                    adjusted_curr[j] = theta_max[j];
                }
                else{
                    adjusted_curr[j] = theta_min[j];
                }
            }
...
std::pair<bool, std::vector<std::vector<double>>> RobotCtrl::findFollowPath(const Eigen::Matrix4d& aim_mat,
    ...
    std::shared_ptr<Body> part = robot_cpoy_->getPart(part_name);
    Eigen::Matrix4d part_base = robot_cpoy_->getPartBase(part_name);
    Eigen::Matrix4d start_mat = robot_cpoy_->getEefByName(part_name);
    std::vector<double> start_theta = part->getTheta();
    std::vector<std::vector<double>> thetas;
    
    // 第一步检查目标是否可达
    std::pair<bool, std::vector<std::vector<double>>> back_pair = robot_cpoy_->controlPart(aim_mat, part_name, ignore_angle);
    ...
    if(back_pair.first){
        for(int i = 0; i < 10; i++){
            ...
            std::verctor<std::tuple<double, double, double>> path = planPath(start, end);
            ...
            if(path.size() > 0){
                ...
                for (auto point : path){
                    ...
                    std::pair<bool, std::vector<std::vector<double>>> pair = robot_cpoy_->controlPart(aim_tmp, part_name, ignore_angle);

                    if(pair.first){
                        thetas_can.append(pair.second.back());
                    }
                }
        ...
        thetas.append(back_pair.second.back());
...
std::pair<bool, std::vector<std::vector<double>>> RobotCtrl::goAim(const Eigen::Matrix4d& aim_mat,const std::string& part_name = "body",
    bool ignore_angle = false){

        return back_pair = robot_cpoy_->controlPart(aim_mat, part_name, ignore_angle);
}
...
std::pair<bool, std::vector<std::vector<double>>> RobotCtrl::cooperationJoint(const std::string& main_name, const std::string& sub_name,
     const std::vector<double>& theta_input){
        ...
        for ( auto i : theta_input){
            ...
            if(pair.first){
                sub_theta_can.append(pair.second.back());
            }
            else{
                find_path = false;
                break;
...
std::pair<bool, std::map<std::string, std::vector<std::vector<double>>>> RobotCtrl::cooperationBody(
     const std::vector<std::string>& sub_name, const std::vector<double>& theta_input){
        syncBodyAndCopy();
        std::string main_name = "body";
        std::shared_ptr<Body> main_part = robot_copy_->getPart(main_name);
        
        std::map<std::string, std::shared_ptr<Body>> sub_parts;
        std::map<std::string, std::vector<std::vector<double>> sub_theta_can;
        ...
        for ( auto i : theta_input){
            ...
            for(auto name : sub_name){
                ...
                if(pair.first){
                    sub_theta_can[name].append(pair.second.back());
                    sub_theta[name] = pair.second.back();
                }
                else{
                    sub_theta_can[name].append(sub_theta[name]);
                    find_path = false;
                    break;
                }
            }
...
std::vector<std::vector<double>> RobotCtrl::jointInterpolation(const std::vector<double>& thetas,
    ...
    double speed){
        ...
        std::vector<std::vector<double>> thetas_can = std::vector<std::vector<double>>();
        thetas_can.append(part->getTheta());
        // 这里检查一下角度序列长度是否一致
